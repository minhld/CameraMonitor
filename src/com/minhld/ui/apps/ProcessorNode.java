...
public class ProcessorNode extends Thread {
	JFrame mainFrame;
	JTextField ipText;
	...
	public void run() {
		...
		mainFrame.addWindowListener(new WindowAdapter() {
			...
			public void windowClosing(WindowEvent e) {
				int response = JOptionPane.showConfirmDialog(ProcessorNode.this.mainFrame, 
...
//		startInitObjects();
	...
	private JToolBar buildToolBar() {
		...
		findPadBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (ProcessorNode.this.isAuto) {
					MoveInstructor.move(0, 0);
				} 
				ProcessorNode.this.isAuto = !ProcessorNode.this.isAuto;
				controlInfoText.setText("AUTOMATION IS " + (ProcessorNode.this.isAuto ? "SET" : "CLEARED"));
				findPadBtn.setText(ProcessorNode.this.isAuto ? "Stop Finding" : "Find Pad");
		...
		initMovingBtn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				ProcessorNode.this.isMovingNode = !ProcessorNode.this.isMovingNode;
				// controlInfoText.setText("AUTOMATION IS " + (RosAutoRed.this.isMovingNode ? "SET" : "CLEARED"));
				initMovingBtn.setText(ProcessorNode.this.isMovingNode ? "Stop Node" : "Start Moving Node");
...
//		slidesPanel.setBorder(new EmptyBorder(15, 15, 15, 15));
		...
		buttonPanel.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent e) {
				...
				if (ProcessorNode.this.isServerInUsed) {
					ProcessorNode.this.buttonPanel.requestFocusInWindow();
				}
			}
			...
			public void run() {
				...
				ROSUtils.execute(graphCameraName, new CameraNode(new CameraNode.ImageListener() {
					...
					public void imageArrived(Image image) {
						...
						ProcessorNode.this.processTimeLabel.setText("Displaying Time: " + drawTime + "ms | " +  
						...
						ProcessorNode.this.topicInfoText.setText("Distance: " + AppUtils.getNumberFormat(objectDistance) + "ft(s)\n" + 
	...
	private void setFindingPadStatus(boolean isAuto) {
		ProcessorNode.this.isAuto = isAuto;
		controlInfoText.setText("AUTOMATION IS " + (ProcessorNode.this.isAuto ? "SET" : "CLEARED"));
		findPadBtn.setText(ProcessorNode.this.isAuto ? "Stop Finding" : "Find Pad");
